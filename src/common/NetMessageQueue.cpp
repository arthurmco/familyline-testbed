...
NetMessageQueue::NetMessageQueue(socket_t sock, struct in_addr addr) : sockfd(sock), addr(addr) {}

void NetMessageQueue::SendTCP(const char* m) { write(this->sockfd, m, strlen(m)); }

/* Set/get if the server manager will check the headers of this message
   (i.e, if it starts with '[TRIBALIA') */
bool NetMessageQueue::CheckHeaders() const { return check_headers; }

void NetMessageQueue::SetCheckHeaders(bool val) { check_headers = val; }
...
size_t NetMessageQueue::ReceiveTCP(char* m, size_t len)
{
    ...
    if (peek > 0) buffer_ptr_recv += peek;
...
void NetMessageQueue::SendUDP(UDPMessage m) { udp_buffer_send.push(m); }

bool NetMessageQueue::PeekUDP(UDPMessage& m)
{
    if (udp_buffer_recv.size() == 0 || !udp_init) return false;
...
void NetMessageQueue::InjectMessageUDP(UDPMessage m) { udp_buffer_recv.push(m); }

bool NetMessageQueue::RetrieveSendUDP(UDPMessage& m)
{
    if (udp_buffer_send.size() == 0 || !udp_init) return false;
...
bool NetMessageQueue::InitUDP(struct in_addr udp_addr, int port)
{
    ...
    if (this->udp_socket < 0) return false;

    if (bind(this->udp_socket, (struct sockaddr*)&this->udp_addr, sizeof(this->udp_addr)) < 0) {
        return false;
    }
...
bool NetMessageQueue::CheckUDP() const { return this->udp_init; }

socket_t NetMessageQueue::GetUDPSocket() const { return this->udp_socket; }

socket_t NetMessageQueue::GetSocket() const { return this->sockfd; }

struct in_addr NetMessageQueue::GetAddress() const { return this->addr; }
