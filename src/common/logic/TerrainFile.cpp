...
TerrainFile::TerrainFile(const char* path)
{
    ...
    if (!this->fTerrain) {
        throw terrain_file_exception("File failed to open", path, errno);
    }
}
...
Terrain* TerrainFile::GetTerrain(int index)
{
    ...
    if (tfhdr.magic != TERRAIN_MAGIC_WORD) {
        throw terrain_file_exception("Invalid magic word", fPath, 0);
    }

    if (tfhdr.version != 1) {
        throw terrain_file_exception("Invalid version", fPath, 0);
    }

    if (tfhdr.game != 0) {
        throw terrain_file_exception("Invalid game", fPath, 0);
    }

    if (tfhdr.thdr_offset < sizeof(TerrainFileHeader)) {
        throw terrain_file_exception("Invalid terrain header offset", fPath, 0);
    }
    ...
    if (tdh.width == 0 || tdh.height == 0) {
        throw terrain_file_exception("Invalid size", fPath, 0);
    }

    auto& log = LoggerService::getLogger();

    log->write(
        "terrain-file", LogType::Info, "%s is a %ux%u terrain", fPath, tdh.width, tdh.height);
    ...
    log->write(
        "terrain-file", LogType::Info, "loading %.3f MB of data for slots",
    ...
    if (read_data < tdh_area) {
        delete[] slots;
        throw terrain_file_exception("Unexpected end of file", fPath, 0);
    }
...
TerrainFile::~TerrainFile() { fclose(fTerrain); }
