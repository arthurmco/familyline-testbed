...
int main(int argc, char const* argv[])
{
    ...
    try {
        ...
        while (continue_main) {
            ...
            for (auto cli : clis) {
                ...
                if (cli->GetStatus() >= CS_CONNECTED) {
                    ...
                    if (cli->GetQueue()->PeekTCP(m, 40)) {
                        if (!strncmp(m, "[TRIBALIA GAME READY]", 21) && !cli->IsReady()) {
                        ...
                        if ((!strncmp(m, "[TRIBALIA GAME READY]", 21) && cli->IsReady()) ||
                            (!strncmp(m, "[TRIBALIA GAME NOTREADY]", 21) && !cli->IsReady())) {
                            cli->GetQueue()->ReceiveTCP(m, 40);
                            Log::GetLog()->Warning(
                                "server",
                                "%s set ready status more than one time -- possible flooding?",
                                cli->getName());
                            continue;
                        }
                    ...
                    if (clis.size() > 0 && cli_ready && !game_starting) {
                        ...
                        if (!cli_ready) game_starting = false;
                    }
                    ...
                    while (auto cm = chm->CheckMessage(cli)) {
                        cl.Push(cm);
                        for (auto ccli : clis) {
                            if (ccli->GetID() != cli->GetID()) chm->SendAll(ccli, cli, cm->message);
                        }
                    }
                }
            }
            ...
            clis.remove_if(
                [](Client* c) { return c->IsClosed() || c->GetStatus() == CS_DISCONNECTED; });
        }
