...
TCPConnectionInitiator::TCPConnectionInitiator(PlayerManager* pm) : _pm(pm) {}

/* Add a client to the TCP client initialization. */
void TCPConnectionInitiator::AddClient(Client* c)
{
    if (c->GetStatus() == CS_DISCONNECTED) c->AdvanceStatus();
...
{
    if (_initClients.size() == 0) return;

    char recvbuffer[256];
    memset((char*)recvbuffer, 0, 255);

    for (auto&& tinit : _initClients) {
        if (tinit.cli->GetStatus() >= CS_CONNECTED) continue;
        ...
        switch (tinit.step) {
            ...
            case VersionQueried:
...

                {
                    ...
                    int ret = sscanf(recvbuffer, "[%s %s %u.%u", sname, sver, &vmaj, &vmin);

                    if (strcmp(sname, "TRIBALIA") || strcmp(sver, "VERSION")) {
                        continue;
                    }
                    ...
                    if (vmaj != 0 || vmin != 1) {
                        fprintf(
                            stderr, "Client %s has unsupported version (%u.%u)",
            ...
            case CapabilityQueried:
...

                {
                    ...
                    int ret = sscanf(recvbuffer, "[%s %s", sname, scap);
                    if (ret < 2) {
                        continue;
                    }

                    if (strcmp(sname, "TRIBALIA") || strcmp(scap, "CAPS?]")) {
                        continue;
                    }
            ...
            case PlayerInfoRetrieved:
...

                {
                    ...
                    int ret =
                        sscanf(recvbuffer, "[%s %s %s %d", sname, sinfo, splayername, &playerxp);
                    if (ret < 4) continue;

                    if (strcmp(sname, "TRIBALIA") || strcmp(sinfo, "PLAYERINFO")) {
                        continue;
                    }
                ...
                printf(
                    "%s is connected (%d loops, %d).\n", tinit.cli->getName(), tinit.iters,
                    tinit.cli->GetStatus());
    ...
    _initClients.erase(
        std::remove_if(
            ...
            [](TCPInit c) {
                return c.cli->IsClosed() || c.step == TCPInitStep::ClientReady ||
...
bool TCPConnectionInitiator::HasClient() { return (_initClients.size() > 0); }
