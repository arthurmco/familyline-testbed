...
bool AdminCommandParser::Listen()
{
    ...
    if (bind(this->sock, (sockaddr*)&addr, sizeof(addr))) {
        // Bind failed
        Log::GetLog()->Fatal("admin-command", "Bind failed: %s", strerror(errno));
        return false;
    }

    if (listen(this->sock, 4)) {
        // Listen failed
        Log::GetLog()->Fatal("admin-command", "Listen failed: %s", strerror(errno));
        return false;
    }

    Log::GetLog()->Write("admin-command", "Socket listening at 0.0.0.0:%d", SERVER_PORT);
    return true;
}
...
bool AdminCommandParser::ProcessPlayerListRequest(socket_t clisocket)
{
    ...
    char smsg[2048];
    sprintf(smsg, "[TRIBALIA RESPONSE PLAYERS %zu ", this->_spm->GetPlayers().size());
    for (const auto& pl : this->_spm->GetPlayers()) {
        char pmsg[128];
        snprintf(
            pmsg, 128, "%d %s %d %s %s ", pl.cli->GetID(), pl.cli->getName(), 0,
            (pl.cli->GetStatus() >= CS_CONNECTED) ? "connected" : "connecting", "192.168.0.1");
        strcat(smsg, pmsg);
    }
    strcat(smsg, "]\n");
...
bool AdminCommandParser::ProcessChatListRequest(socket_t clisocket)
{
    ...
    msg[s] = '\0';
    if (strncmp(msg, "[TRIBALIA REQUEST CHAT", 22)) return false;  // Not the same message
    ...
    Log::GetLog()->Write(
        "admin-command-parser", "Requested messages since epoch %lld", since_epoch);
    ...
    for (auto m : messages) {
        char* mchat = new char[strlen(m->message) + 128];
        sprintf(
            mchat, "[TRIBALIA CHAT %ld %d %s %s %zu %s ]", m->time.count(),
            (m->sender) ? m->sender->GetID() : 0, (m->sender) ? m->sender->getName() : "admin",
...
bool AdminCommandParser::SendChat(socket_t clisocket)
{
    ...
    msg[s] = '\0';
    if (strncmp(msg, "[TRIBALIA SEND-CHAT", 19)) return false;  // Not the same message
    ...
    sscanf(msg, "%s %s %s %d %n", mtr, mse, desttype, &msgsize, &msgoffset);
...
void AdminCommandParser::ProcessRequests()
{
    ...
    struct pollfd* fds = new struct pollfd[fdslen];
    ...
    if (r < 0) {
        // Error occurred
        Log::GetLog()->Warning("admin-command", "poll failed: %s", strerror(errno));
        return;
    } else if (r == 0) {
    ...
    for (i = 0; i < fdslen - 1; i++) {
        if (fds[i].revents & POLLHUP) {
            ...
            Log::GetLog()->Write(
                "admin-command", "connection from interface %d closed by remote host\n",
                fnGetInterfaceID(i));
        ...
        if (fds[i].revents & POLLIN) {
            ...
            if (this->ProcessPlayerListRequest(fds[i].fd)) continue;

            if (this->ProcessChatListRequest(fds[i].fd)) continue;

            if (this->SendChat(fds[i].fd)) continue;
    ...
    if (fds[serversock].revents & POLLIN) {
        ...
        while (!continue_con) {
            ...
            if (!strcmp("[TRIBALIA INTERFACE OK ]\n", msg)) continue_con = true;
        }

        char ipstr[INET_ADDRSTRLEN];
        const char* ipaddr = inet_ntop(AF_INET, (void*)&(raddr.sin_addr), ipstr, raddrlen);
        Log::GetLog()->Write(
            "admin-command", "New interface detected, ip %s, id %d", ipaddr, si.id);
...
AdminCommandParser::~AdminCommandParser()
{
    ...
    if (this->sock >= 0) shutdown(this->sock, SHUT_RDWR);
