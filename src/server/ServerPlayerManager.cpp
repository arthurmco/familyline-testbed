...
void PlayerManager::Process()
{
    ...
    for (const auto player : _players) {
        if (player.cli->IsClosed() || player.cli->GetStatus() == CS_DISCONNECTED) {
            continue;
        }

        if (!player.cli->GetQueue()->PeekTCP(rstr, 21)) continue;
    ...
    _players.remove_if([](auto& player) {
        return player.cli->IsClosed() || player.cli->GetStatus() == CS_DISCONNECTED;
    });
}
...
{
    Log::GetLog()->Write(
        "player-manager", "client %s with id %d added, notifying others\n", c->getName(),
        c->GetID());
}
...
void PlayerManager::SendMapList(Client* c)
{
    ...
    c->GetQueue()->SendTCP("[TRIBALIA MAPS 1 | default 'Default map' 0 ]\n");
}
