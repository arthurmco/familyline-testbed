...
{
    if (addr->s_addr == htonl(INADDR_LOOPBACK)) return true;  // loopback is always valid

    if (serv_net_addr_base.s_addr == INADDR_ANY) return true;  // any address, always valid
...
do_retrieve_client:
    ...
    if (!this->CheckAddress(&(cliaddr.sin_addr))) {
        printf(
            "Socket refused: not in the same network "
            "(fd %d), address %s\n",
            clisockfd, ipstr);
        return nullptr;
    }
    ...
    if (sread == 0) {
        if (blocks) goto do_retrieve_client;
    ...
    if (sstr.find("[TRIBALIA CONNECT]\n") != std::string::npos) {
        ...
        if (write(clisockfd, connback, strlen(connback)) > 0) {
            std::shared_ptr<Client> c = std::make_shared<Client>(clisockfd, cliaddr.sin_addr);
            Log::GetLog()->InfoWrite(
                "server-manager", "Client added (fd %d), address %s\n", clisockfd, ipstr);
            clients.push_back(c);
            return c.get();
        }
    }
            ...
            if (pfds[i].revents) {
                if (pfds[i].revents & POLLIN) {
                    ...
                    if (readnum == 0) {
                        ...
                        Log::GetLog()->Write(
                            "server-manager", "Client %d (%s) disconnected", cli->GetID(),
                            cli->getName());
                    ...
                    if (cli->CheckHeaders() && strncmp(readbuf, "[TRIBALIA", 9) != 0) {
                ...
                if (pfds[i].revents & POLLHUP) {
                    auto& cli = clients[i];
                    Log::GetLog()->Write(
                        "server-manager", "Client %d (%s) disconnected (POLLHUP)", cli->GetID(),
                        cli->getName());
