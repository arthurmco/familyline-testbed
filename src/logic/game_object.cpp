...
std::shared_ptr<GameObject> GameObject::create()
{
	...
	cloned->cContainer = this->cContainer ? std::make_optional<ContainerComponent>(this->cContainer.value())
		: std::optional<ContainerComponent>();
	cloned->cLocation = this->cLocation ? std::make_optional<LocationComponent>(this->cLocation.value())
		: std::optional<LocationComponent>();
	cloned->cMovement = this->cMovement ? std::make_optional<MovementComponent>(this->cMovement.value())
		: std::optional<MovementComponent>();

	if (cloned->cAttack)
		cloned->cAttack->object = this;

	if (cloned->cLocation)
		cloned->cLocation->object = this;
