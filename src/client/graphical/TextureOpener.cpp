...
TextureFile* TextureOpener::TextureOpenBMP(FILE* f, const char* path) {
    ...
    if (fread((void*)bmp_header, 1, 14, f) < 14) {
        log->write("texture-opener", LogType::Warning,
                   "Unexpected EOF in BMP header while opening %s", path);
        return nullptr;
    }

    if (bmp_header[0] != 'B' && bmp_header[1] != 'M') {
        log->write("texture-opener", LogType::Warning,
                   "Invalid magic number while opening %s", path);
        return nullptr;
    }
    ...
    if (off_pixels == 0) {
        log->write("texture-opener", LogType::Warning,
                   "Wrong pixel header while opening %s", path);
        return nullptr;
    }
    ...
    if (fread((void*)dib_header, 1, 40, f) < 40) {
        log->write("texture-opener", LogType::Warning,
                   "Unexpected EOF in DIB header while opening %s", path);
        return nullptr;
    }
    ...
    if (width == 0 || height == 0 || bpp == 0) {
        log->write("texture-opener", LogType::Warning,
                   "Invalid size while opening %s", path);
        return nullptr;
    }
...
{
    auto& log = LoggerService::getLogger();
    log->write("texture-opener", LogType::Info,
               "Opening %s", path);

    FILE* f = fopen(path, "rb");

    if (!f) {
        log->write("texture-opener", LogType::Warning, "File %s not found"
                   " (error %d)", path, errno);
        return nullptr;
    }
    ...
    if (ilLoad(IL_TYPE_UNKNOWN, path) == IL_FALSE) {
        ...
        log->write("texture-opener", LogType::Error, "Error '%s' while opening %s",
                   estr, path);
        return nullptr;
    }
