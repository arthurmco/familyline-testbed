...
*/
static TerrainBitmap MergeTerrain(
    TerrainBitmap lt, TerrainBitmap rt, TerrainBitmap lb, TerrainBitmap rb)
{
    /*
      lt------rt
      |        |
...
#define GET_TEXTURE_RAW(fn) \
    (unsigned int*)TextureOpener::OpenFile(fn)->GetTextureRaw(0, 0, ImageHeight, ImageWidth)

TerrainBitmap TerrainRenderer::GetTerrainTexture(unsigned int type_id)
{
    auto& log                       = LoggerService::getLogger();
    static unsigned int* textures[] = {GET_TEXTURE_RAW("textures/terrain/grass.png")};
...
#pragma omp parallel for
    for (unsigned sy = 0; sy < SlotSide - 1; sy++) {
        for (unsigned sx = 0; sx < SlotSide - 1; sx++) {
            ...
            auto srcs = MergeTerrain(
                GetTerrainTexture(texture_lt), GetTerrainTexture(texture_rt),
    ...
    return new Texture(
        ImageWidth * SlotSide, ImageHeight * SlotSide, GL_RGBA, terrain_surface.data());
}
