...
void LightManager::AddLight(Light* l) { LightManager::_lights.push_back(l); }

/* Removes a light */
void LightManager::Remove(Light* l)
{
    std::remove_if(LightManager::_lights.begin(), LightManager::_lights.end(), [l](Light* curr) {
        return l->getName() == curr->getName();
    });
}

/* Removes all lights */
void LightManager::RemoveAll() { LightManager::_lights.clear(); }

/*
 * Get the best lights, the strongest ones, from a circle that
 * has the center in 'çenter' and radius 'radius'
 */
std::list<Light*> LightManager::GetBestLights(glm::vec3 center, double radius, const int max_lights)
{
    ...
    for (auto i = 0; i < max_lights; i++) {
        ...
        for (auto light = lights.begin(); light != lights.end(); light++) {
            ...
            log->write(
                "light-manager", LogType::Debug, "light %s has score %.3f", (*light)->getName(),
                score);
