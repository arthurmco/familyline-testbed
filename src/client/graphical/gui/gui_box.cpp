...
std::string GUIBox::describe() const {
  char v[96];
  sprintf(v, "GUIBox: size %d x %d, pos: %d, %d (%zu elements): [ ", width_,
          height_, x_, y_, controls_.size());
 ...
 */
void GUIBox::update() {
  bool dirty = dirty_ || std::any_of(controls_.begin(), controls_.end(),
                                     [](GUIControl *c) { return c->dirty(); });
  if (!dirty)
    return;
...
void GUIBox::receiveInput(const GUIEvent &e) {
//  char s[128];
//  sprintf(s, "echo \"input received for focused index %zd (%zd) %zd\" >> a.txt",
//          focused_index_, tab_index_, previous_tab_index_);
//  system(s);
...
void GUIBox::initLayout(BaseLayout &layout) {
  layout.initialize(
      [this]() {
        std::vector<int> ids;
        std::transform(this->controls_.begin(), this->controls_.end(),
                       std::back_inserter(ids),
                       [](auto *c) { return c->id(); });
        return ids;
      },
      [this](int id) -> GUIControl * {
        auto v = std::find_if(this->controls_.begin(), this->controls_.end(),
                              [id](auto *c) { return c->id() == id; });
        if (v == this->controls_.end())
          return nullptr;
