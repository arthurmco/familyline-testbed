...
GUICanvas GUIImageControl::openImageFromFile(const char* filename)
{
    ...
    log->write("gui-image-control", LogType::Debug, "opening image '%s'",
               filename);

    auto status = cairo_surface_status(image_surface);
    switch (status) {
    case CAIRO_STATUS_NO_MEMORY:
        log->write("gui-image-control", LogType::Error,
                   "unsufficient memory while opening %s",
                   filename);
        return nullptr;

    case CAIRO_STATUS_FILE_NOT_FOUND:
        log->write("gui-image-control", LogType::Error, "'%s' has not been found",
                   filename);
        return nullptr;

    case CAIRO_STATUS_READ_ERROR:
        log->write("gui-image-control", LogType::Error, "error while reading '%s'",
                   filename);
        return nullptr;
    default:
...
bool GUIImageControl::processSignal(GUISignal s)
{
    ...
    case ContainerAdd:
        ...
        log->write("gui-image-control", LogType::Debug, "image has size %.2f x %.2f",
                   imageW, imageH);
