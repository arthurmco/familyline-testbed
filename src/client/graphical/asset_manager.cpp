...
Asset AssetManager::processAsset(AssetItem& av)
{
    ...
    std::transform(av.dependencies.begin(), av.dependencies.end(),
                   std::back_inserter(asset.dependencies),
                   [&](std::shared_ptr<AssetItem>& adep) {
                       return this->_assets[adep->name];
                   });
    ...
    if (av.type == "mesh") {
        ...
        auto texfile = std::dynamic_pointer_cast<TextureFile>(
            asset.loadAssetObject()[0]);
        Texture* tex = texfile->GetTextureCut(0,0,-1,-1);

        asset.object = std::make_optional(texfile);
        GFXService::getTextureManager()->AddTexture(
            asset.name.c_str(), tex);
