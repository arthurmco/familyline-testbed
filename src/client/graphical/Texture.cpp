...
Texture::Texture(int width, int height, GLenum format, void* data)
{
    ...
    if ((glerr = glGetError()) != GL_NO_ERROR) {
        log->write("texture", LogType::Error,
    ...
    if ((glerr = glGetError()) != GL_NO_ERROR) {
        log->write("texture", LogType::Error,
                   "GL error %#x while creating texture handle %d",
                   glerr, tex_handle);
    }

    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
    log->write("texture", LogType::Debug,
               "Created texture with %dx%d, format %#x, with id %d",
                 ...
                 glm::vec2 uv_size)
{
    ...
    LoggerService::getLogger()->write(
        ...
        "Created texture from handle %d (%dx%d), cutted with uv-pos (%.2f, %.2f) and uv-size (%.2f, %.2f)",
...
size_t Texture::GetMaximumSize()
{
    ...
    if (texture_size == -1) {
        ...
        log->write("texture", LogType::Info,
                   "Discovered texture max size: %dx%d", texture_size, texture_size);

        if (texture_size < 0) {
            log->write("texture", LogType::Fatal,
                       "Invalid texture max size. Something is wrong");            
        }
