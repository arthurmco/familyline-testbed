...
TEST(ObjectAttack, TestMeleeAttack){
  ...
  auto defc1 = std::optional<AttackComponent>(
      AttackComponent { nullptr, 1.0f, 1.0f, 1.0f, 1.0f, 1.0f, 1.0f, 1.0f,
                        1.0f, 0.0f, 2.0f, 1.0f, 3.14f});

  auto atker = make_object(
      {"obj1", "Object 1", glm::vec2(5, 5), 200, 200, true, []() {}, atkc1});

  auto defer = make_object(
      {"obj2", "Object 2", glm::vec2(5, 5), 200, 200, true, []() {}, defc1});

  ASSERT_TRUE(atker->getAttackComponent()->isInAttackRange(
      defer->getAttackComponent().value()));

  auto res = atker->getAttackComponent()->doDirectAttack(
      defer->getAttackComponent().value());
...
TEST(ObjectAttack, TestDeathNotification) {
    ...
    auto atker_s = make_object(
        {"obj1", "Object 1", glm::vec2(5, 5), 200, 200, true, []() {}, atkc1});

    auto defer_s = make_object(
        {"obj2", "Object 2", glm::vec2(5, 5), 200, 200, true, []() {}, defc1});
...
TEST(ObjectAttack, NotAttackIfMeleeOutOfRange) {
  ...
  ASSERT_FALSE(atker->getAttackComponent()->isInAttackRange(
      defer->getAttackComponent().value()));
