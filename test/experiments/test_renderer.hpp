...
class TestControlPaintData : public ControlPaintData
{
public:
    TestControlPaintData(GUIControl& control, int width, int height, int x, int y,
                         std::vector<std::unique_ptr<TestControlPaintData>> children = {})
        : control(control), width(width), height(height), x(x), y(y),
 ...
 */
class TestRenderer : public GUIRenderer
{
public:
...
class TestControlPainter : public GUIControlPainter
{
public:
    TestControlPainter(TestRenderer &cr) : cr_(cr) {}
