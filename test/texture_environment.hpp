...
struct Texture {
  ...
  Texture(surface_unique_ptr data)
      : data(std::move(data)), renderer_handle(std::nullopt),
        texture_unit(std::nullopt), refcount(0) {}
};
...
public:
   ...
   */
  virtual tl::expected<std::unique_ptr<Texture>, ImageError>
      loadTextureFromFile(std::string_view) = 0;
   ...
   */
  virtual tl::expected<std::unique_ptr<Texture>, ImageError>
  loadTextureFromMemory(std::span<uint8_t> data, size_t width, size_t height,
   ...
   * Returns the API-specific texture code on success, or an error on failure.
   */
  virtual tl::expected<uintptr_t, ImageError>
  setTextureData(Texture &, std::span<uint8_t> data, size_t width,
                 size_t height, TextureFormat format) = 0;
   ...
   * will be rendered
   */
  virtual tl::expected<uintptr_t, TextureError> bindTexture(Texture &,
                                                            unsigned unit) = 0;
   ...
   */
  virtual tl::expected<uintptr_t, TextureError>
  unbindTexture(unsigned unit) = 0;
